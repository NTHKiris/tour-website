...
@section('content')
    <div class="max-w-7xl mx-auto py-10">
        <h1 class="text-2xl font-bold mb-6 text-cyan-700">Quản lý tour</h1>
        @if(session('success'))
                ...
                @foreach($tours as $tour)
                    <div class="flex items-center justify-between bg-white rounded-lg shadow px-5 py-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('tours.destroy', $tour) }}" method="POST" class="delete-form">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="showDeleteModal(this.closest('form'), '{{ $tour->title }}')"
