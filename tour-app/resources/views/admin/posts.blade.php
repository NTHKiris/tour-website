...
@section('content')
    <div class="max-w-7xl mx-auto py-10">
        <h1 class="text-2xl font-bold mb-6 text-cyan-700">Quản lý bài viết</h1>
        @if(session('success'))
            ...
            <div class="space-y-4">
                @foreach($posts as $post)
                    <div class="flex items-center justify-between bg-white rounded-lg shadow px-5 py-4">
                        <div>
                            <div class="font-semibold text-lg text-gray-900">{{ $post->title }}</div>
                            <div class="text-sm text-gray-500">
                                <i class="fa-solid fa-folder-open mr-1"></i>
                                {{ $post->category->name ?? 'Chưa phân loại' }}
                                <span class="ml-2"><i
                                        class="fa-solid fa-user mr-1"></i>{{ $post->author->name ?? 'Ẩn danh' }}</span>
                            </div>
                        </div>
